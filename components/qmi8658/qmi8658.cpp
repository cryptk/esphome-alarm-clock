...
void QMI8658Component::update() {
  ...
  // float ay_g = ay / 1.0f;
  // float az_g = az / 1.0f;
  // float gx_dps = gx / 1.0f;
  // float gy_dps = gy / 1.0f;
  // float gz_dps = gz / 1.0f;
  
  // Publish accelerometer sensor values
  if (this->accel_x_sensor_ != nullptr)
    this->accel_x_sensor_->publish_state(1);
  if (this->accel_y_sensor_ != nullptr)
    this->accel_y_sensor_->publish_state(2);
  if (this->accel_z_sensor_ != nullptr)
    this->accel_z_sensor_->publish_state(3);

  // Publish gyroscope sensor values
  if (this->gyro_x_sensor_ != nullptr)
    this->gyro_x_sensor_->publish_state(4);
  if (this->gyro_y_sensor_ != nullptr)
    this->gyro_y_sensor_->publish_state(5);
  if (this->gyro_z_sensor_ != nullptr)
    this->gyro_z_sensor_->publish_state(6);
}
